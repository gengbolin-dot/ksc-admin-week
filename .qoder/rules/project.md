...
### 线上服务器
- **服务器地址**: 120.92.123.190
- **SSH连接**: 使用 `ssh -o BatchMode=yes -o ConnectTimeout=10` 确保命令可靠
- **文件传输**: 使用 `scp -r` 上传文件
- **系统日期基准**: 2026年5月3日

### 本地联调配置（直连线上PostgreSQL）
- **数据库地址**: `postgresql://admin:Kingsoft0531@120.92.123.68:39944/project_codebuddy?sslmode=disable`
- **白名单**: PG有IP白名单，本机公网IP需联系DBA加入白名单后才能连通
- **OIDC认证配置**:
  - **本地联调**: 禁用OIDC (`VITE_ENABLE_OIDC=false`)，使用模拟用户ID (`VITE_MOCK_USER_ID=31784`)
...
- **启动命令**:
  ...
  ```
- **验证连通**: `nc -zv -w 5 120.92.123.68 39944`
...
线上服务器 `120.92.123.190` 历史上**同时存在两套前端入口**，构建时间会漂移导致用户访问到旧版本：

| 入口 URL | 提供方 | dist 路径 | 工作目录 |
|---|---|---|---|
| `http://120.92.123.190` (:80) | Nginx (`/etc/nginx/conf.d/project-management.conf`) | `/opt/project-management/dist` | — |
| `http://120.92.123.190:5173` | `vite preview` 后台进程 | `/opt/codebuddy/dist` | `/opt/codebuddy` |

**用户实际访问入口**：`.env.production` 中 `VITE_FRONTEND_URL=http://120.92.123.190:5173`，即 **:5173 才是用户入口**，:80 仅作为备份/Nginx 通道。**两边 dist 必须保持一致**，否则会出现「OKR 不加粗、风险提示不显示」等"代码改了线上看不到"的现象。
...
2. **同步两份 dist**（必须同时覆盖，缺一不可）：
   ...
   ssh root@120.92.123.190 "mv /opt/codebuddy/dist /opt/codebuddy/dist.bak.$TS && cp -r /opt/project-management/dist /opt/codebuddy/dist"
   ```
3. **重启 :5173 vite preview 进程**（vite preview 是静态服务，文件名带 hash 时浏览器强刷即可生效，但保险做法是重启）：
   ```bash
   ssh root@120.92.123.190 "pkill -f 'vite preview --host 0.0.0.0 --port 5173' || true; \
...
4. **校验关键特征字（grep 法）**：minify 后变量名会丢失，但字符串字面量保留，必须验证下列关键字在线上 JS 中存在：
   ```bash
   JS=$(curl -s http://120.92.123.190:5173/ | grep -oE 'index-[A-Za-z0-9_-]+\.js' | head -1)
   for kw in scheduleChanges delayRisks memberAlerts 本周排期空闲人员 font-bold; do
     echo "$kw -> $(curl -s http://120.92.123.190:5173/assets/$JS | grep -c "$kw")"
   done
   ```
